...
function defaultLanguage (language, defaultLanguage) {
      ...
      .on('createEditor', () => {
        ...
        const init = () => {
          ...
          const toggleEditable = () => {
            let res = doc.body.getAttribute('contenteditable')
            if (editor.getMode() !== MODE_SOURCE && !editor.getReadOnly()) {
            let res = doc.body.setAttribute('contenteditable', true.toString())
            } else {
              doc.body.hasAttribute('contenteditable') && doc.body.removeAttribute('contenteditable')
            }
            return res
          };
          ...
          if (docMode) {
            ...
            editor.e
              .on('beforeGetNativeEditorValue', () =>
                clearMarkers(
                  editor.o.iframeDoctype +
                  doc.documentElement.outerHTML
                )
              )
              .on('beforeSetNativeEditorValue',
                ({ value }) => {
                  if (editor.isLocked) {
                    return false;
                  }
            ...
            const resizeIframe = editor.async.throttle((...args) => {
              editor.async.requestAnimationFrame(() => {
                if (
                  ...
                  editor.iframe.style.height = `${editor.editor.offsetHeight + marginOffset}px`
                }
              });
...
class IframeSandboxPatch {
  ...
  setSandbox(editor){
    editor.iframe?.setAttribute("sandbox", "allow-same-origin")
  }
}

export default function(Jodit) {
  Jodit.plugins.add('iframe-sandbox', IframeSandbox);
  // new IframeSandbox().init(Jodit)
}
