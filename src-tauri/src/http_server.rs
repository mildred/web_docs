...
#[tauri::command]
fn serve_file(name: &str, _content: &str) -> String {
...
pub fn listen<R: Runtime>(
  ...
  handler: tauri::CallbackFn,
) {
    let function_name = &format!("window['_{}']", handler.0);
    let js = "(function(){
    })()";
    webview.eval(js)
}
...
#[derive(Template)]
#[default_template("./http_server.js")]
...
pub fn init<R: Runtime>() -> TauriPlugin<R> {
    ...
    PluginBuilder::new("os")
        .js_init_script(init_js.to_string())
        .invoke_handler(tauri::generate_handler![serve_file])
        .build()
}
